...
pygame.display.set_caption('Snake Game - Exponential Search')
...
class SnakeGame:
    ...
    def exponential_search(self):
        # Exponential search implementation to find path to food
        ...
        # Sort moves by distance for exponential search
        ...
        # Exponential search for the best move
        if len(possible_moves) == 1:
            return possible_moves[0][1]
            
        bound = 1
        while bound < len(possible_moves) and possible_moves[bound][0] < possible_moves[0][0]:
            bound *= 2
            
        # Perform binary search in the identified range
        result_idx = binary_search(
            possible_moves, 
            possible_moves[0][0],
            bound // 2,
            min(bound, len(possible_moves)-1)
        )
        
        return possible_moves[result_idx][1]

    def update(self):
        if self.game_over:
            return

        # Find next move using exponential search
        next_move = self.exponential_search()
        if next_move:
            head_x, head_y = self.snake[0]
